...
# プロンプト例と出力イメージ
- プロンプト例: 「付与された論文と補助資料を読み、分野外の聴衆にも伝わるよう一般的な課題紹介から始まる15分発表のslides_script.mdを作成してください。アウトラインはHook→Problem→Gap→Approach→Method→Results→Interpretation→Takeawaysの順にし、各スライドはAssertion–Evidence形式で書いてください。」
- 出力イメージ（抜粋）:
  ```
  ## Story Outline
  - 1) なぜセキュリティ評価は複雑化しているのか
  - 2) 従来手法の限界と盲点
  - 3) 提案手法: 軽量な自動評価フレームワーク
  ...

  ## Slide-by-slide Script
  ### Slide 1: ソフトウェアが増えるほど攻撃面は指数的に広がる
  - On-slide: 近年のサービス数推移グラフ、攻撃件数の推移、セキュリティ人材不足の数字
  - Speaker script: 近年サービス数が増え攻撃機会が急増していること、専門家不足で検証が追いつかないという一般的背景から話し始める。
  - Transition: 「この一般的な課題の中で、特に問題になるのが…」
  ```
