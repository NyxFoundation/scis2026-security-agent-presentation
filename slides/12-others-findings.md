...
<!--
こちらは他の参加者が発見した高・中重大度の脆弱性の一覧です。我々はこれらを全て見逃しました。

見逃し原因を分類すると、4件は仕様の細部の見落としでした。キャッシュキーに何を含めるべきか、入力長さの検証など、仕様に書いてあるが見落としやすい部分です。

2件はタイミング/並行性の問題で、Fork遷移やmetadata更新のタイミングに起因します。

1件は暗号ライブラリc-kzg-4844の実装誤りで、これは意図的にスコープ外としていました。

次のスライドで、これらの見逃し原因をグラフで整理します。
-->
